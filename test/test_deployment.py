...
def mock_runner_proxy_config(config: Config, pod: PodData) -> Path:
    """Generate a job-runner.toml config for the mock GitHub API"""
    return generate_config(config,
                           forge_opts=f'api-url = "{GHAPI_URL_POD}"',
...
def make_pr_event_commands(github_env: str) -> str:
    return f'''set -ex
      {github_env}
...
def test_mock_pr_url_proxy(config: Config, pod: PodData, bots_sha: str, mock_github,
